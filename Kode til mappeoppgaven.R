...
#WDIsearch(string = "growth", field = "", short = TRUE, cache = NULL)
...
df_rest0<-WDI(
  country = "all",
  indicator = c('poptot'="SP.POP.TOTL", 'gi'="NE.GDI.FTOT.KD.ZG", 'gx'="NE.EXP.GNFS.KD.ZG", 'nry'="NY.ADJ.DRES.GN.ZS", 'p'="SP.POP.GROW", 'lifeExp'="SP.DYN.LE00.IN" ),  
  start = 2000,
  end = 2019,
  extra = TRUE,
  cache = NULL,
...
df_rest <- subset(df_rest, select = c("country", "region", "income", "iso3c", "iso2c", "year", "poptot", "p", "nry", "gi", "gx","lifeExp"))
...
col_order <- c("country",  "region", "income", "iso3c", "iso2c", "year", "gdppc", "gdppc0", "poptot", "p", "avg_n", "avg_nsy", "nry", "gi", "gx", "avg_educ", "LifeExp")
...
df_growth <- subset(df_growth, select = c("country",  "region", "income", "iso3c", "iso2c","year", "poptot", "gdppc", "gdppc0", "avg_gdpgrowth", "avg_n", "avg_p", "avg_nsy", "avg_nry", "avg_gi", "avg_gx", "avg_educ", "lifeExp"))
...
df <- subset(df_growth2019, select = c("avg_gdpgrowth", "avg_gi", "avg_nsy", "avg_nry", "avg_gx", "avg_p", "avg_educ", "gdppc"))


  
# Endre navnene på variablene i datasettet
colnames(df) <- c("Vekst i BNP", "Vekst i investeringer", "Sparing som andel av BNI (netto)", 
                  "Årlig reduksjonsrate i naturressurser", "Årlig vekstrate i eksport", "Vekstrate i befolkningen",
                  "Gjennomsnittlig antall år i skole (befolkning 15+)", "BNP per innbygger")

# Lag tabellen
st(df, vars = c("Vekst i BNP", "Vekst i investeringer", "Sparing som andel av BNI (netto)", 
                "Årlig reduksjonsrate i naturressurser", "Årlig vekstrate i eksport", "Vekstrate i befolkningen",
                "Gjennomsnittlig antall år i skole (befolkning 15+)", "BNP per innbygger"),
   labels = colnames(df),
   summ = list(
   ...
   summ.names = list(
     c('N','Gjennomsnitt','SD','Minimum','Maksimum'), #Dette setter titler til kolumnene.
     c('N','Gjennomsnitt')
   ))                        
...
#df <- df %>% filter(avg_gi<100)


  sparing1 <- df_growth2019 %>% 
    filter(avg_gi < 100) %>%  
    ggplot(aes(x = avg_nsy, y = log(gdppc))) + 
    ggtitle("Sammenhengen mellom BNP per innbygger \n og sparing  ") + 
    xlab("Sparing som andel av BNI(Netto) %") + 
    ylab("BNP per innbygger 2000-2019 (log)") + 
  ...
  befolkningsvekst <- df_growth2019 %>% 
    filter(avg_gi < 100) %>%  
    ggplot(aes(x = avg_p, y = log(gdppc))) + 
    ggtitle("Sammenhengen mellom BNP per innbygger \n og vekstrate i befolkningen  ") + 
    xlab("Årlig vekstrate i befolkningen %") + 
    ylab("BNP per innbygger 2000-2019 (log)") + 
    geom_text(aes(label=iso3c),size=3) + 
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_rect(fill = 'white'))+
    theme_classic() 
  ...
  humankapital1 <- df_growth2019 %>% 
    filter(avg_gi < 100) %>%  
    ggplot(aes(x = avg_educ, y = log(gdppc))) + 
    ggtitle("Sammenhengen mellom BNP per innbygger \n og antall år på skolen  ") + 
    xlab("Gjennomsnittlig antall år på skolen (15+)") + 
    ylab("BNP per innbygger 2000-2019 (log)") + 
    geom_text(aes(label=iso3c),size=3) + 
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_rect(fill = 'white'))+
    theme_classic() 
  ...
  sparing <- df_growth2019 %>% 
    filter(avg_gi < 100) %>%  
    ggplot(aes(x = avg_nsy, y = avg_gdpgrowth)) + 
    ggtitle("Sammenhengen mellom gj.snittlig årlig vekst i BNP per innbygger \n og vekstrate i investeringer ") + 
    xlab("Sparing som andel av BNI (netto) %") + 
    ylab("Årlig vekstrate i BNP per innbygger 2000-2019") + 
    geom_text(aes(label=iso3c),size=3) + 
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_rect(fill = 'white'))+
    theme_classic() 
  ...
  humankapital <-  df_growth2019 %>% 
    filter(avg_gi < 100) %>%  
    ggplot(aes(x = avg_educ, y = avg_gdpgrowth)) + 
    ggtitle("Sammenhengen mellom gj.snittlig årlig vekst i BNP per innbygger \n og gj.snitt antall år på skole ") + 
    xlab("Gjennomsnitt antall år på skole (15+)") + 
    ylab("Årlig vekstrate i BNP per innbygger 2000-2019") + 
    geom_text(aes(label=iso3c),size=3) + 
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_rect(fill = 'white'))+
    theme_classic() 
...
#avg_gdpgrowth", "avg_gi", "avg_nsy", "avg_nry", "avg_gx", "avg_p", "avg_educ", "poptot", "gdppc"))
...
View (no_outliers1)
model2 <- lm(avg_gdpgrowth  ~ avg_gi + avg_nsy + avg_nry + avg_gx + avg_p + avg_educ + gdppc0,  data= no_outliers1) #NB: Du må legge inn "Dine" variabler her
summary(model2)
...
labs13 <- c("Vekst i BNP","Årlige vekstrate i investeringer", "Sparing som andel av BNI", 
            "Årlige reduksjonsrate i naturressurser", "Årlig vekstrate i eksport", 
            "Vekstrate i befolkningen", "Gjennomsnittlig antall år i skole (15+)",
            "BNP per innbygger")


tab_model(model2, p.style = "stars", pred.labels = labs13, title = "Gjennomsnittlig BNP vekst")



#plot med oversikt over BNP per innbygger (log) og forventet levealder
lifep <- df_growth2019 %>% 
  ggplot(aes(x = lifeExp, y = log(gdppc))) + 
  ggtitle("Sammenhengen mellom BNP per innbygger \n og Levealder  ") + 
